...
def upload_brown(register_plugin=False):
    ...
    reading_times = data.fdur
    voc_text_ID = data.text_id
    voc_word_num = data.Word_Number
    voc_word = data.word
    code = data.code
    sentid = data.sentid
    sentpos = data.sentpos
    docid = data.docid
    startofsentence = data.startofsentence
    endofsentence = data.endofsentence
    wlen = data.wlen
    resid = data.resid
    time = data.time
    evid = data.evid
    ...
    # reading_times = np.empty((r_dim,))
    # # reading_times[:] = np.nan
    # voc_text_ID = np.empty((r_dim,))
    # # voc_text_ID[:] = np.nan
    # voc_word_num = np.empty((r_dim,))
    # # voc_word_num[:] = np.nan
    # voc_word = np.empty((r_dim,), dtype=str)
    # # voc_word[:] = np.nan
    # subject = np.empty((r_dim,), dtype=str)
    # code = np.empty((r_dim,))
    # # code[:] = np.nan
    # sentid = np.empty((r_dim,))
    # # sentid[:] = np.nan
    # sentpos = np.empty((r_dim,))
    # # sentpos[:] = np.nan
    # docid = np.empty((r_dim,), dtype=str)
    # # docid[:] = np.nan
    # startofsentence = np.empty((r_dim,), dtype=bool)
    # # startofsentence[:] = np.nan
    # endofsentence = np.empty((r_dim,), dtype=bool)
    # # endofsentence[:] = np.nan
    # wlen = np.empty((r_dim,))
    # # wlen[:] = np.nan
    # resid = np.empty((r_dim,))
    # # resid[:] = np.nan
    # time = np.empty((r_dim,))
    # # time[:] = np.nan
    # evid = np.empty((r_dim,), dtype=str)
    # evid[:] = np.nan
    ...
    #     reading_times[index] = d['fdur']

    #     voc_text_ID[index] = d['text_id']
    #     voc_word_num[index] = d['Word_Number']
    #     voc_word[index] = d['word']

    #     # subject[index] = str(d['subject'])
    #     # print(subject[index])
    #     code[index] = d['code']
    #     sentid[index] = d['sentid']
    #     sentpos[index] = d['sentpos']
    #     docid[index] = d['docid']
    #     startofsentence[index] = d['startofsentence']
    #     endofsentence[index] = d['endofsentence']
    #     wlen[index] = d['wlen']
    #     resid[index] = d['resid']
    #     time[index] = d['time']
    #     evid[index] = d['evid']
    ...
    assembly = BehavioralAssembly(reading_times,
                                  ...
                                  coords={'word': ('presentation', voc_word),
                                          ...
                                          'subject_id': ('presentation', subjects),
                                          ...
                                          'evid': ('presentation', evid)
                                        #   'subject_id': ('subject', subjects),
                                        #   'subject_id1': ('subject', subjects)
                                          })
